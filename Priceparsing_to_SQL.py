...
# Добавление в SQL индекса торговой сессии
rows = c.execute('SELECT * FROM Prices ORDER BY Symbol, Date').fetchall()
column_names = [i[1] for i in c.execute('PRAGMA table_info("Prices")').fetchall()]
table = []
for row in rows:
...
for index_temp in range(41,index):
    ...
    query = f'select p.Symbol,p.Date, p.Close, p.Volume, p.WDayIndex, (p.Volume/p.Close*(p.Close-c2.Close)' \
            ...
            f'from (select * from Prices where WDayIndex={index_temp}) as p ' \
            f'join (select * from Prices where WDayIndex={index_temp-1}) as c2 on c2.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-2}) as c3 on c3.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-3}) as c4 on c4.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-4}) as c5 on c5.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-5}) as c6 on c6.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-6}) as c7 on c7.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-7}) as c8 on c8.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-8}) as c9 on c9.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-9}) as c10 on c10.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-10}) as c11 on c11.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-11}) as c12 on c12.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-12}) as c13 on c13.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-13}) as c14 on c14.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-14}) as c15 on c15.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-15}) as c16 on c16.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-16}) as c17 on c17.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-17}) as c18 on c18.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-18}) as c19 on c19.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-19}) as c20 on c20.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-20}) as c21 on c21.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-21}) as c22 on c22.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-22}) as c23 on c23.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-23}) as c24 on c24.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-24}) as c25 on c25.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-25}) as c26 on c26.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-26}) as c27 on c27.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-27}) as c28 on c28.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-28}) as c29 on c29.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-29}) as c30 on c30.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-30}) as c31 on c31.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-31}) as c32 on c32.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-32}) as c33 on c33.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-33}) as c34 on c34.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-34}) as c35 on c35.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-35}) as c36 on c36.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-36}) as c37 on c37.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-37}) as c38 on c38.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-38}) as c39 on c39.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-39}) as c40 on c40.Symbol = p.Symbol ' \
            f'join (select * from Prices where WDayIndex={index_temp-40}) as c41 on c41.Symbol = p.Symbol ' \
            f'join (select Symbol, avg(Volume) as Vol from Prices where WDayIndex>={index_temp-80} ' \
            f'and WDayIndex<={index_temp} group by Symbol) as v on v.Symbol = p.Symbol'
